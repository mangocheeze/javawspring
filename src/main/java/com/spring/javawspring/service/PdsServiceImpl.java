...
public class PdsServiceImpl implements PdsService {
	...
	public void setPdsInput(MultipartHttpServletRequest mFile, PdsVO vo) {
		try {
			List<MultipartFile> fileList = mFile.getFiles("file"); //여러개의 파일
			String oFileNames = "";
			String sFileNames = "";
			int fileSizes = 0;
			
			for(MultipartFile file : fileList) {
				String oFileName = file.getOriginalFilename(); //업로드할때 파일이름
				String sFileName = saveFileName(oFileName); 
				
				//파일업로드 메소드 호출
				writeFile(file, sFileName); //한번에 올리기
			...
			vo.setFSize(fileSizes); //해도되고안해도됨
	...
	private void writeFile(MultipartFile file, String sFileName) throws IOException {
		byte[] data = file.getBytes(); //new 새로생성안함. 
	...
	private String saveFileName(String oFileName) {
		...
		fileName += cal.get(Calendar.MILLISECOND); //줘도되고안줘도되고
