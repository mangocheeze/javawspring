      ...
      <tr>
        ...
        <c:forEach begin="1" end="${lastDay}" varStatus="st">
          ...
          <td id="td${cnt}" ${todaySw==1 ? 'class=today' : ''} style="font-size:0.9em">
            ...
            <a href="scheduleMenu?ymd=${ymd}"> <!-- ymd변수에는 년월일들어가있어야함 -->
            	${st.count}<br/>  <!-- 달력의 날짜가 나옴 -->
              ...
              <c:forEach var="vo" items="${vos}">
                <c:if test="${fn:substring(vo.SDate,8,10)==st.count}">
                  <c:if test="${vo.part != tempPart}">  <!-- 모임이 temp에 들어가있는게 같지않냐? 같지않으면 아래실행 같으면 빠져나옴-->
                    <c:if test="${tempSw != 0}"> <!-- 여길 들어가지않게끔설계.빠져나옴  -->
	                    - ${tempPart}(${tempCnt})건<br/>
	                    <c:set var="tempCnt" value="0"/>
                    </c:if>
                    <c:set var="tempPart" value="${vo.part}"/>                  
                  </c:if>
                  <c:set var="tempSw" value="1"/> <!-- 스위치  -->
                  <c:set var="tempCnt" value="${tempCnt + 1}"/> <!-- 카운트 -->
                </c:if>
            	</c:forEach>
