...
<head>
  ...
  <script>
    ...
    $(document).ready(function(){
    	$("#reply").show();
    	$("#replyViewBtn").hide(); //댓글보이기버튼 처음에는 안보이게하기
    	
    	$("#replyHiddenBtn").click(function(){ //댓글가리기버튼클릭시
    		$("#reply").slideUp(500);
    		$("#replyViewBtn").show();
    		$("#replyHiddenBtn").hide();
    	});
    	
    	$("#replyViewBtn").click(function(){ //댓글보이기버튼클릭시
    		$("#reply").slideDown(500); //0.5초 가리기
    		$("#replyViewBtn").hide();
    		$("#replyHiddenBtn").show();
    	});
    });
    ...
    */
    ...
    function insertReply(idx, level, levelOrder, nickName) {
    	...
    	$("#replyBoxCloseBtn"+ idx).show(); //닫기버튼
    	$("#replyBox"+idx).slideDown(500); //부드럽게 내리기
    ...
    function closeReply(idx) {
    	...
    	$("#replyBoxCloseBtn"+ idx).hide(); //닫기버튼
    	$("#replyBox"+idx).slideUp(500); //부드럽게 내리기
    }
    ...
    function replyCheck2(idx, level, levelOrder) {
    	...
    	let content = $("#content" + idx).val(); //위 두줄이랑같은말
...
<%-- 	        <c:if test="${preVo.preIdx != 0}">
	        ...
	        <c:if test="${!empty pnVos[1]}"> <!-- 다음글이 없지 않으면 실행 -->
	        다음글 : <a href="${ctp}/board/boardContent?idx=${pnVos[1].idx}&pageSize=${pageSize}&pag=${pag}">${pnVos[1].title}</a><br/>
	        </c:if>
	        <!-- 이전글 처리때문에 추가된 루틴 -->
	        <c:if test="${vo.idx < pnVos[0].idx}"> <!-- 제일첫번째글은 sql 에서 0번방이 이전글이 다음글내용이 되어버리니까 현재게시글 idx가 이전글idx보다 작으면 -->
...
<div id="reply" class="container">
	<table class="table table-hover text-left">
	  ...
	  <c:forEach var="replyVo" items="${replyVos}">
	    <tr>
	      <td class="text-left"> <!-- 왼쪽으로 밀어야 대댓글이 옆으로 밀림 -->
	      	<c:if test="${replyVo.level <= 0}">${replyVo.nickName}</c:if> <!-- 부모댓글의 경우는 들여쓰기 하지않는다. -->
	        <c:if test="${replyVo.level > 0}"> <!-- 부모댓글의 댓글이나 대댓글의 경우 -->
	        	<c:forEach var="i" begin="1" end="${replyVo.level}">&nbsp;&nbsp;</c:forEach><!-- 레벨만큼 들여쓰기 -->
	        	└▷ ${replyVo.nickName}
	        </c:if>
	    ...
	    <tr>
	    	<td colspan="5" class="m-0 p-0" style="border-top:none;"><div id="replyBox${replyVo.idx}"></div></td> <!-- replyBox옆에 idx를 넣어서 중복되지않게함 -->
	    </tr>
	  </c:forEach>
	</table>
	<!-- 댓글 입력창 -->
	<%-- <form name="replyForm" method="post" action="${ctp}/board/boardReplyInput"> ajax처리할거라 안씀--%>
	<form name="replyForm">
