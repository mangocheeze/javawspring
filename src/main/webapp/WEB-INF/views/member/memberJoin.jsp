  ...
  <script>
    ...
    function fCheck() {
      ...
      let tel3 = myform.tel3.value;
    	...
    	let ext = fName.substring(fName.lastIndexOf(".")+1);	// 파일 확장자 발췌 /뒤에서부터 .만날때까지 그리고 거기서 +1한거부터 마지막까지를 잘라내 비교할거임(jpg만 잘라올거임)
    	let uExt = ext.toUpperCase();		// 확장자를 대문자로 변환//위에서 잘라온걸 대문자로 바꿈(jpg랑 JPG둘다올수있어서 JPG로 통일시키려고)
    ...
    function idCheck() {
    	...
    	//아이디가 정상적으로 왔을때 ajax 로 중복이냐 아니냐확인
    	$.ajax({
    		type : "post",
	  ...
	  function nickCheck() {
	  	...
	  	if(nickName.trim() == "" || nickName.length<1 || nickName.length>=20) {
	  		alert("닉네임을 확인하세요!(닉네임은 1~20자 이내)");
	  		myform.nickName.focus();
	  		return false;
	  	}
	  	//닉네임이 정상적으로 왔을때 ajax 로 중복이냐 아니냐확인
	  	$.ajax({
	  		type : "post",
...
<!--   <form name="myform" method="post" class="was-validated" enctype="multipart/form-data"> 파일업로드가 하나라도 들어가면  enctype써주기-->
    <h2>회 원 가 입</h2>
    <br/>
